...
npx -y app-store-connect-api-mcp@latest
```
...
```json
{
  "mcpServers": {
    "app_store_connect_api_api": {
      "command": "npx",
      "args": ["-y", "app-store-connect-api-mcp", "--client=claude", "--tools=dynamic"],
      "env": {
...
| --------------------------------- | ------------------------ | ---------------- |
| `x-app-store-connect-api-api-key` | `apiKey` | itc-bearer-token |
