...
describe('resource relationships', () => {
  ...
  test.skip('listMatchmakingQueues: request options and params are passed correctly', async () => {
    ...
    await expect(
      client.gameCenterMatchmakingRuleSets.relationships.listMatchmakingQueues(
        'id',
        { limit: 200 },
  ...
  test.skip('listRules: request options and params are passed correctly', async () => {
    // ensure the request options are being passed correctly by passing an invalid HTTP method in order to cause an error
    await expect(
      client.gameCenterMatchmakingRuleSets.relationships.listRules(
        'id',
        { limit: 200 },
        { path: '/_stainless_unknown_path' },
      ),
  ...
  test.skip('listTeams: request options and params are passed correctly', async () => {
    // ensure the request options are being passed correctly by passing an invalid HTTP method in order to cause an error
    await expect(
      client.gameCenterMatchmakingRuleSets.relationships.listTeams(
        'id',
        { limit: 200 },
        { path: '/_stainless_unknown_path' },
      ),
