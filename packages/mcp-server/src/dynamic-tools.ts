...
export function dynamicTools(endpoints: Endpoint[]): Endpoint[] {
  ...
  const listEndpointsTool = {
    ...
    tool: {
      ...
      description: 'List or search for all endpoints in the App Store Connect API TypeScript API',
  ...
  const getEndpointTool = {
    ...
    tool: {
      ...
      description:
        'Get the schema for an endpoint in the App Store Connect API TypeScript API. You can use the schema returned by this tool to invoke an endpoint with the `invoke_api_endpoint` tool.',
      inputSchema: zodToInputSchema(getEndpointSchema),
    },
    handler: async (client: AppStoreConnectAPI, args: Record<string, unknown> | undefined) => {
      if (!args) {
  ...
  const invokeEndpointTool = {
    ...
    tool: {
      ...
      description:
        'Invoke an endpoint in the App Store Connect API TypeScript API. Note: use the `list_api_endpoints` tool to get the list of endpoints and `get_api_endpoint_schema` tool to get the schema for an endpoint.',
